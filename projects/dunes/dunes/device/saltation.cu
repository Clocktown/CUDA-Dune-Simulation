...
// This is an outdated version of Saltation that was not used in the Paper. It can still be set in the UI by setting the Saltation Mode to "Per Frame".
...
__global__ void saltationKernel(Array2D<float2> t_terrainArray, const Array2D<float2> t_windArray, Array2D<float4> t_resistanceArray, Buffer<float> t_slabBuffer)
{
	...
	const float abrasion{ terrain.y < c_parameters.abrasionThreshold ? 
						  ...
						  (1.0f - terrain.y / c_parameters.abrasionThreshold) * scale : 0.0f };
