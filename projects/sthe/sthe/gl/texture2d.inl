...
{
	GL_CHECK_ERROR(glGetTextureSubImage(getHandle(), t_mipLevel, 0, 0, 0, t_width, t_height, 0, t_format, t_type, static_cast<GLsizei>(t_destination.size() * sizeof(T)), t_destination.data()));
}
...
{
	GL_CHECK_ERROR(glGetTextureSubImage(getHandle(), t_mipLevel, t_x, t_y, 0, t_width, t_height, 0, t_format, t_type, static_cast<GLsizei>(t_destination.size() * sizeof(T)), t_destination.data()));
}
...
inline void Texture2D::download(T* const t_destination, const int t_width, const int t_height, const GLenum t_format, const GLenum t_type, const int t_mipLevel) const
{
	...
	GL_CHECK_ERROR(glGetTextureSubImage(getHandle(), t_mipLevel, 0, 0, 0, t_width, t_height, 0, t_format, t_type, size, t_destination));
}
...
{
	const GLsizei size{ t_width * t_height * static_cast<GLsizei>(sizeof(T)) };
	GL_CHECK_ERROR(glGetTextureSubImage(getHandle(), t_mipLevel, t_x, t_y, 0, t_width, t_height, 0, t_format, t_type, size, t_destination));
}
